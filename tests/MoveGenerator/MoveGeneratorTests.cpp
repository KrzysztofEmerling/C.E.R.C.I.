    ...
    static void RunPerftTests()
    {
        ...
        for (const auto &testCase : testCases)
        {
            ...
            for (const auto &[depth, expectedNodes] : testCase.expectedNodes)
            {
                ...
                uint64_t nodes = Debug::PerftCount(state, depth);
        ...
        std::cout << "Total preft test time: " << testModuleTimeEnd - testModuleTimeStart << "\n";
        std::cout << "-------------------------------------------------------------" << std::endl;
    }
};